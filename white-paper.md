...
The architecture is logically divided into four main domains:
   ...
   - **Agent Workflows ({{site.data.keyword.wxorchestrate_full_notm}})**: The core intelligence layer responsible for interpreting user intent and coordinating actions across internal and external services. Agent workflows are defined using Watson orchestrate to interpret the user intent, breakdown complex tasks into steps and decide which tools and models to invoke. {{site.data.keyword.wxorchestrate_full_notm}} supports Prompts that define the instructions, context, and constraints given to the Agents. Knowledgebase is a structured and unstructured information used by the Agents for LLM responses using RAG. Knowledgebase can a local vector store in {{site.data.keyword.wxorchestrate_full_notm}} or you can integrate to external Vector Store.
